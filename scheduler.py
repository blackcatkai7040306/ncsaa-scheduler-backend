...
class ScheduleOptimizer:
    ...
    def _schedule_division(self, division: Division, teams: List[Team]) -> List[Game]:
        ...
        # For each matchup, for each slot, create a variable indicating if game is scheduled
        for (i, j) in matchups:
            game_vars[(i, j)] = {}
            for slot_idx in range(num_slots):
                var_name = f'game_t{i}_t{j}_s{slot_idx}'
                game_vars[(i, j)][slot_idx] = model.NewBoolVar(var_name)
        ...
        for team_idx in range(num_teams):
            ...
            for (i, j) in matchups:
                if i == team_idx or j == team_idx:
                    for slot_idx in range(num_slots):
                        team_games.append(game_vars[(i, j)][slot_idx])
        ...
        for (i, j) in matchups:
            model.Add(sum(game_vars[(i, j)][slot_idx] for slot_idx in range(num_slots)) <= 1)
        ...
        for slot_idx in range(num_slots):
            for team_idx in range(num_teams):
                ...
                for (i, j) in matchups:
                    if i == team_idx or j == team_idx:
                        games_at_slot.append(game_vars[(i, j)][slot_idx])
        ...
        for team_idx in range(num_teams):
            ...
            for slot_idx, slot in enumerate(self.time_slots):
                ...
                slots_by_week[week_num].append(slot_idx)
                ...
                for (i, j) in matchups:
                    if i == team_idx or j == team_idx:
                        for slot_idx in week_slots:
                            if slot_idx in game_vars[(i, j)]:
                                games_in_week.append(game_vars[(i, j)][slot_idx])
        ...
        for (i, j) in matchups:
            ...
            for slot_idx in range(num_slots):
                if slot_idx in game_vars[(i, j)]:
                    objective_terms.append(game_vars[(i, j)][slot_idx] * score)
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            for (i, j) in matchups:
                for slot_idx in range(num_slots):
                    if slot_idx in game_vars[(i, j)] and solver.Value(game_vars[(i, j)][slot_idx]):
                        team1 = teams[i]
                        team2 = teams[j]
                        slot = self.time_slots[slot_idx]
    ...
    def _greedy_schedule_division(self, division: Division, teams: List[Team]) -> List[Game]:
        ...
        # Pre-filter time slots by division requirements
        usable_slots = []
        ...
        for score, team1, team2 in matchups:
            ...
            for slot in usable_slots:
                ...
                # Check if teams can play on this date
                can_play = True
            ...
            for pass_num in range(max_passes):
                ...
                for team in teams_needing_games:
                    ...
                    for opponent in teams:
                        ...
                        if is_rematch:
                            # Allow more rematches as passes increase
                            max_rematches = 3 if pass_num >= 10 else 2
                            if team_games_count[team.id] < 6 or team_games_count[opponent.id] < 6:
                                # Count how many rematches we've already done
                                rematch_count = sum(1 for r in rematches_used if r[0] == matchup_key)
                                if rematch_count >= max_rematches:
                                    continue
                            else:
                                # Allow rematches if teams need to reach 8
                                if team_games_count[team.id] < target_games or team_games_count[opponent.id] < target_games:
                                    if rematch_key in rematches_used and pass_num < 15:
                                        continue
